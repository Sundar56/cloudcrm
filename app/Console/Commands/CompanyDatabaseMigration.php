...
class CompanyDatabaseMigration extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:company-database-migration';
     ...
     * @var string
     */
    protected $description = 'Company Database Migration and files creation';
    ...
    {
        $runningMigrationCount = OtherJobs::where('status', '1')->count();

        if($runningMigrationCount == 0){

            $companies = OtherJobs::where('status', '0')->limit(1)->orderBy('id','asc')->get();

            if ($companies->isEmpty()) {
                $this->info('No Company with status 0 found.');
                return;
            }else{

                foreach ($companies as $company) {

                    $payload = json_decode($company->payload, true);

                    if (isset($payload['company_id']) && isset($payload['company_name'])) {
                        ...
                        $this->info("Running migration for company ID: $companyId, Company Name: $companyName");
                        $this->info("Migration started at:". now());

                        $this->createCompanyDatabase($companyId, $companyName,$company);
                        $this->info("Migration ended at:". now());

                        $company->status = '2'; 
                        $company->save();
                    } else {
                        $this->error("Invalid payload format for company ID: {$company->company_id}. Missing company_id or company_name.");
                         $company->status = '3'; 
                         $company->save();
                    }
                }
    ...
    /**
     * Method for creating the company database.
     */
    private function createCompanyDatabase($companyId, $companyName,$company)
    {
        // set_time_limit(0);
        ...
        $roles = Role::where('company_id', $companyId)->get(['id', 'name']);
        $insertData = [];
        ...
        DB::connection($dbName)->table('cloud_sso_userlevels')->insert($insertData);
    ...
    public function copyCompanyFiles($companyName, $companyId, $channel)
    {
        ...
        // $sourcePath = 'F:\vignesh\crmcompanies\basesetup';
        ...
        try {
            ...
            $this->createApiConfigFile($cloudCrmFolderPath . DIRECTORY_SEPARATOR . 'apiconfig.php',$companyId);
            $this->createApiConfigFile($cloudRouterProfileFolderPath . DIRECTORY_SEPARATOR . 'apiconfig.php',$companyId);
            $this->createApiConfigFile($cloudSSOPath . DIRECTORY_SEPARATOR . 'apiconfig.php',$companyId);
        } catch (\Exception $e) {
    ...
    private function createApiConfigFile($filePath,$companyId)
    {
        try {
...
//php artisan app:company-database-migration
